...
async function loadPolyfillPlugins() {
	return [[(await import("babel-plugin-polyfill-corejs3")).default, {
		method: "usage-global",
...
function parseVersionCore(v) {
	return v.split("-", 1)[0].split(".").map((part) => Number.parseInt(part, 10) || 0);
}
...
function isVersionGte(version, minVersion) {
	...
	for (let i = 0; i < 3; i++) if (core[i] !== minCore[i]) return core[i] > minCore[i];
...
function resolveLegacyOutputMinify(minify, supportsOxc, target) {
	if (!(supportsOxc && (minify === "oxc" || minify === true))) return false;
	if (target === void 0 || target === false) return true;
	return { compress: { target } };
}
function resolveLegacyBuildMinify(minify, supportsOxc) {
	return supportsOxc && (minify === "oxc" || minify === true) ? "oxc" : minify ? "terser" : false;
}
function viteLegacyPlugin(options = {}) {
	...
	let supportsLegacyOxcMinification;
	return [
		legacyConfigPlugin,
		{
			...
			async generateBundle(opts, bundle) {
				...
				if (!isLegacyBundle(bundle)) {
					...
					await buildPolyfillChunk(this, config.mode, modernPolyfills, bundle, facadeToModernPolyfillMap, config.build, "es", opts, true, genLegacy, supportsLegacyOxcMinification);
					return;
				}
				if (!genLegacy) return;
				...
				if (legacyPolyfills.size || !options.externalSystemJS) {
					...
					await buildPolyfillChunk(this, config.mode, legacyPolyfills, bundle, facadeToLegacyPolyfillMap, config.build, "iife", opts, options.externalSystemJS, false, supportsLegacyOxcMinification, "es2015");
				}
			}
		},
		{
			...
			configResolved(_config) {
				...
				if (!supportsLegacyOxcMinification && config.build.minify === "oxc") config.logger.warn(import_picocolors.default.yellow(`'oxc' minifier is not supported for legacy chunks by Vite version ${viteVersion}. Please upgrade to Vite version ${legacyOxcMinificationSupportedVersion} or later.`));
				...
				const createLegacyOutput = (options = {}) => {
					return {
						...
						minify: resolveLegacyOutputMinify(config.build.minify, supportsLegacyOxcMinification, "es2015")
					};
				};
			...
			transformIndexHtml(html, { chunk, bundle }) {
				...
				if (chunk.fileName.includes("-legacy")) {
					...
					if (config.environments.client.build.chunkImportMap) facadeToLegacyImportMap.set(chunk.facadeModuleId, bundle[getImportMapFilename(config)]);
					if (genModern) return;
				}
...
async function buildPolyfillChunk(ctx, mode, imports, bundle, facadeToChunkMap, buildOptions, format, rollupOutputOptions, excludeSystemJS, prependModenChunkLegacyGuard, supportsLegacyOxcMinification, overrideMinifyCompressTarget) {
	const { assetsDir, sourcemap, target } = buildOptions;
	const minifyCompressTarget = overrideMinifyCompressTarget ?? target;
	const minify = resolveLegacyBuildMinify(buildOptions.minify, supportsLegacyOxcMinification);
		...
		build: {
			...
			rolldownOptions: {
				...
				output: {
					...
					minify: resolveLegacyOutputMinify(buildOptions.minify, supportsLegacyOxcMinification, minifyCompressTarget)
				}
			}
		},
		esbuild: false,
